...
def build_executable():
    ...
    output_name = f"EasyTierLite-Server-{platform_name}"
    ...
    cmd = [
        ...
        "--hidden-import", "action.configs",
        "--hidden-import", "action.services",
        "--hidden-import", "action.peers",
        "--hidden-import", "action.androids",
...
def copy_output(output_name):
    ...
    # 创建输出目录
    output_dir = PROJECT_ROOT / "EasyTier-Lite" / "app" / "bin"
    output_dir.mkdir(parents=True, exist_ok=True)
    
    # 复制文件
    if source.exists():
        target = output_dir / f"EasyTierLite-Server{ext}"
